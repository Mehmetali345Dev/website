  ...
  <v-app>
    <v-navigation-drawer v-if="$device.isMobile" v-model="drawer" temporary fixed app>
      <v-list>
        <v-list-item>
          <v-list-item-content>
            <v-btn disabled text>Mehmetali_345 | Blog</v-btn>
          </v-list-item-content>
        </v-list-item>
    ...
    <v-app-bar dense app>
      <v-container :class="{ 'd-flex': true, 'mobile-container': $device.isMobile }">
        <v-toolbar-items v-if="$device.isMobile" :style="{ width: '100%', height: '100%' }">
          <v-spacer />
          <v-btn @click.stop="$router.push('/blog')" text>Mehmetali_345 | Blog</v-btn>
          <v-spacer />
        </v-toolbar-items>

        <v-toolbar-items v-else-if="!$device.isMobile">
          <v-btn @click="$router.push('/blog')" :style="{ backgroundColor: '#212121' }">345</v-btn>
          <v-btn to="/blog" exact title="Blog Ana Sayfa" text>Ana Sayfa</v-btn>
          <v-btn to="/blog/gonderi/ara/csharp" text>C#</v-btn>
          <v-btn to="/blog/gonderi/ara/oyun" text>Oyun</v-btn>
          <v-btn to="/blog/gonderi/ara/yazılım" text>Yazılım</v-btn>
          <v-btn to="/blog/gonderi/ara/haber" text>Haber</v-btn>
        </v-toolbar-items>
    ...
    <v-footer absolute app>
      <v-container class="d-flex">
        <span :style="{ alignSelf: 'center' }">
          Mehmet Ali Külahçı 2020  | &copy; 2020 - Tüm hakları saklıdır. Vue.js
          <v-icon class="ml-1" left>mdi-heart</v-icon>Nuxt ile yapıldı. Kodların bir bölümü EGGSY' e aittir.
        </span>
