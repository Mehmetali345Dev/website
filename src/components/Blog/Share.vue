  ...
  <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-3">
    ...
    >
      <input
        ref="share-url"
        readonly
        :value="`https://mehmetali345.xyz${path}`"
    ...
    share(option) {
      if (option === "url") {
        ...
        if (!el) {
          ...
          el.value = this.path ? `https://mehmetali345.xyz${this.path}` : location.href
        ...
        switch (option) {
          case "twitter":
            url = `https://twitter.com/intent/tweet?via=AnakinS07677978&text=${encodeURIComponent(
              this.title + "\n" + location.href
            )}`
            break
          case "telegram":
